...
parser.add_argument('--network', default='unet', choices=['unet', 'hourglass', 'resnet'], type=str, dest='network')
...
## 네트워크 생성
if network == "unet":
    net = UNet(in_channels=in_channels, out_channels=out_channels, nker=nker, learning_type=learning_type).to(device)
elif network == 'hourglass':
    net = Hourglass(in_channels=in_channels, out_channels=out_channels, nker=nker, learning_type=learning_type).to(device)
elif network == 'resent':
    net = ResNet(in_channels=in_channels, out_channels=out_channels, nker=nker, learning_type=learning_type).to(device)
